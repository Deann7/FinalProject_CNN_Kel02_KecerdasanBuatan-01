...
Langkah pra-pemrosesan yang dilakukan mencakup ekstraksi \textit{bounding box} otomatis menggunakan teknik \textit{HSV Masking}. Posisi wajah pengemudi pada dataset ditandai dengan kotak berwarna (hijau, merah, atau biru), sehingga \textit{masking} dilakukan untuk mengambil koordinat ujung $(x_1, y_1, x_2, y_2)$ dari kotak tersebut. Seluruh gambar kemudian di-\textit{resize} ke resolusi $640 \times 640$ piksel. Pembagian data dilakukan dengan proporsi 75\% untuk \textit{training}, 15\% untuk \textit{validation}, dan 10\% untuk \textit{testing}. Untuk memastikan reprodusibilitas hasil, \textit{random seed} diatur pada nilai \textbf{42}.
\end{sloppypar}
...
Sistem VisionGuard dilatih menggunakan arsitektur Faster R-CNN dengan \textit{backbone} ResNet-50 FPN dan bobot awal (\textit{pre-trained weights}) dari dataset COCO \cite{lin2014coco}. Lingkungan eksperimen dijalankan menggunakan \textit{framework} PyTorch (v2.2.2) dan Torchvision (v0.17.2) pada Google Colab dengan GPU Tesla T4 (VRAM 15,6 GB). Pustaka tambahan yang digunakan meliputi OpenCV (v4.10) untuk pemrosesan gambar dan Albumentations (v1.4) untuk \textit{data augmentation}. Tabel~\ref{tab:hyperparam} menyajikan konfigurasi \textit{hyperparameter} yang digunakan selama proses pelatihan model.
...
Eksperimen menunjukkan bahwa proses \textit{training} model berjalan dengan konvergensi yang baik. Evaluasi dilakukan pada setiap \textit{epoch} terhadap \textit{validation set}. Model terbaik diperoleh pada Epoch ke-3 dengan nilai mAP \textit{validation} sebesar 0,8785. Ketika model terbaik ini diuji menggunakan \textit{test set} (6.653 gambar), model menghasilkan nilai mAP@[0.5:0.95] sebesar \textbf{0,8749} dan mAP@0.50 sebesar \textbf{0,9343}.
...
Berdasarkan performa per kelas pada \textit{test set}, kelas \texttt{sleepyCombination} memperoleh skor mAP tertinggi sebesar 0,9175, diikuti oleh \texttt{yawning} (0,8680), \texttt{slowBlinkWithNodding} (0,8572), dan \texttt{notdrowsy} (0,8568). Tingginya skor pada kelas \texttt{sleepyCombination} disebabkan oleh fitur visual yang sangat khas berupa mata yang tertutup sepenuhnya, sementara kelas \texttt{notdrowsy} memiliki skor terendah karena kemiripan visual dengan \texttt{slowBlinkWithNodding} pada \textit{frame} tunggal.
...
Secara kualitatif, model mampu mendeteksi letak wajah pengemudi serta mengklasifikasikan ekspresi \textit{yawning} dan \textit{sleepyCombination} dengan akurat karena adanya fitur visual yang jelas seperti bentuk mulut terbuka lebar atau posisi kepala tertunduk. Namun, terdapat beberapa jenis kesalahan (\textit{error}) yang teridentifikasi:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Kesalahan klasifikasi antara \textit{Not Drowsy} dan \textit{Slow Blink with Nodding}:} Gambar pada kelas \texttt{notdrowsy} terkadang diprediksi sebagai \texttt{slowBlinkWithNodding}. Hal ini terjadi karena gerakan kedipan mata normal yang tidak sengaja terekam pada satu \textit{frame} tunggal dianggap oleh model sebagai kondisi mata tertutup akibat mengantuk. Tanpa konteks temporal, model tidak dapat membedakan durasi kedipan.
...
Keterbatasan utama dari eksperimen ini adalah model hanya bekerja pada \textit{frame} gambar mandiri (fitur spasial 2D) sehingga tidak dapat menganalisis durasi dari suatu gerakan (fitur temporal). Selain itu, evaluasi hanya dilakukan pada satu dataset (NTHU-DDD), sehingga generalisasi model terhadap dataset lain belum dapat diukur. Meskipun demikian, performa deteksi yang dihasilkan sudah sangat memadai sebagai dasar pengembangan sistem keselamatan berkendara.
